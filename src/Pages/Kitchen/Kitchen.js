...
const Kitchen = () => {
	...
	const changePreparingStatus = (newStatus, index) => {
		firebase.firestore().collection('orders').doc(order[index].uid).update({
			status: newStatus,
			...
			<div className="page-order">
				<div className="pendente">
					<div className="button-container page-order conj-btn">
						<Button className="button-cafe" color="primary" name="Pendente" onClick={openPending} />
					</div>
				</div>
				<div className="preparando">
					<div className="button-container page-order conj-btn">
						<Button className="button-cafe" color="primary" name="Preparando" onClick={openPreparing} />
					</div>
				</div>

				{pending &&
					order.filter((item) => item.status === 'Pendente').map((item, index) => (
				...
				{preparing &&
					order.filter((item) => item.status === 'Preparando').map((item, index) => (
						<div className="card-lista">
							<div className="card-titulo">
								<p>Atendente:{item.waiterName}</p>
								<p>Mesa: {item.table}</p>
